...
SELECT 
    ...
    CAST(
        ROUND(
            (COALESCE(SUM(c.compatibilidade), 0) * 100.0) 
            / NULLIF(th.total_horarios_disciplina, 0), 2
        ) AS DOUBLE PRECISION
    ) AS compatibilidade_percent,
    CASE 
...
FROM professor p
JOIN profissional_role pr ON p.id = pr.id
JOIN profissional prof ON pr.profissional_id = prof.id
JOIN pessoa pes ON prof.id = pes.id
...
WHERE p.id IN (
    ...
    WHERE pc.curso_id = (SELECT curso_id FROM disciplina WHERE id = :disciplinaId)
)
GROUP BY 
    p.id, pes.nome, pes.foto, pes.email, pes.telefone, 
    th.total_horarios_disciplina, preferencias.pref_count
ORDER BY 
    CASE 
